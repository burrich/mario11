...
function love.draw()
	-- TODO: implement camera module

	love.graphics.setBackgroundColor(107, 140, 255)

	-- X translation when the player move forward
	local translation = player.x > player.xOrigine

	if translation then
		-- Translation doesn't change at the end of the map
		-- in order to avoid black screen
		if (player.x <= (mapPixelWidth - windowWidth/2 - player.width/2)) then
            translateX  = -(player.x - player.xOrigine) 
		end
